#![allow(dead_code)]

use hex_literal::hex;
use reqwest::blocking::Client;
use serde::{Deserialize, Deserializer, Serialize};

const URL: &str = "https://medieval-adelle-jonistartuplab-17499dda.koyeb.app";
const TEST_URL: &str = "http://localhost:8000";

const CIPHERTEXT: [u8; 128] = hex!("");

#[derive(Serialize, Deserialize, Debug)]
struct EncryptRes {
    server: String,
    cipher_hex: String,

    #[serde(deserialize_with = "string_to_u32")]
    time_ns: u32
}


fn string_to_u32<'de, D>(deserializer: D) -> Result<u32, D::Error>
where
    D: Deserializer<'de>,
{
    let s = String::deserialize(deserializer)?;
    s.parse::<u32>().map_err(serde::de::Error::custom)
}

#[derive(Debug)]
pub struct Attacker {
    client: Client,
    url: String,
}

impl Attacker {
    pub fn new(url: &str) -> Self {
        let client = Client::new();

        Self {
            client,
            url: String::from(url)
        }
    }

    pub fn encrypt(&self, plain: &str, repeat: Option<u32>) -> reqwest::Result<reqwest::blocking::Response> {
        let mut full_url = String::new();
        full_url.push_str(&self.url);
        full_url.push_str(&format!("/encrypt?p={}", plain));

        if let Some(r) = repeat {
            full_url.push_str(&format!("&r={}", r));
        }

        self.client.get(full_url).send()
    }

}


#[test]
fn deserialize() -> reqwest::Result<()>{
    let attacker = Attacker::new(TEST_URL);

    let res = attacker.encrypt("ciccio", None)?;

    let encrypt_res: EncryptRes = serde_json::from_str(&res.text()?).unwrap();

    println!("{:?}", encrypt_res);

    Ok(())
}
