/*
 * Credit to https://github.com/bcgit/bc-java/
 */
...
public class AES {
    ...
    private static int inv_subWord(int x) {
        return (
            Si[x&255]&255 |
            ((Si[(x>>8)&255]&255)<<8) |
            ((Si[(x>>16)&255]&255)<<16) |
    ...
    private int[][] generateWorkingKey(byte[] key, boolean forEncryption) {
        ...
        switch (KC) {
        case 4: {
            int col0 = littleEndianToInt(key,  0);     W[0][0] = col0;
            int col1 = littleEndianToInt(key,  4);     W[0][1] = col1;
            int col2 = littleEndianToInt(key,  8);     W[0][2] = col2;
            int col3 = littleEndianToInt(key, 12);     W[0][3] = col3;
        ...
        case 8: {
            ...
            int col4 = littleEndianToInt(key, 16);     W[1][0] = col4;
            int col5 = littleEndianToInt(key, 20);     W[1][1] = col5;
            int col6 = littleEndianToInt(key, 24);     W[1][2] = col6;
            int col7 = littleEndianToInt(key, 28);     W[1][3] = col7;
     ...
     *
     * @param forEncryption whether the cipher will be used for encryption.
     * @param key the key used to initialize the cipher.
     * @exception IllegalArgumentException if the key is not 16/24/32 bytes.
     */
    ...
    private void encryptBlock(byte[] in, int inOff, byte[] out, int outOff, int[][] KW) {
        ...
        for (int i =0; i<4; i++) {
            // input & key whitening
            C[i] = littleEndianToInt(in, inOff + 4 * i) ^ KW[0][i];
        }
        int[] T = new int[4];
        ...
        // output
        for (int i =0; i<4; i++) {
            intToLittleEndian(C[i], out, outOff +  4 * i);
        }
    }
    ...
    private void decryptBlock(byte[] in, int inOff, byte[] out, int outOff, int[][] KW) {
        ...
        for (int i =0; i<4; i++) {
            // input & key whitening
            C[i] = littleEndianToInt(in, inOff + 4 * i) ^ KW[ROUNDS][i];
        }
        int[] T = new int[4];
